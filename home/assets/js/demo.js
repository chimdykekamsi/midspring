(function($){
   ...
   // Number.MAX_SAFE_INTEGER||(Number.MAX_SAFE_INTEGER=9007199254740991),Number.isSafeInteger=Number.isSafeInteger||function(e){return Number.isInteger(e)&&Math.abs(e)<=Number.MAX_SAFE_INTEGER};
    ...
    // Notify Function
    const innerWidth=$(window).innerWidth();var inotify="0";window.notify=((i,n="err",e=3e3)=>{let t="success",o="Success!",c="fa la-check-circle",f="bottom",q="right";innerWidth<568&&(f="top",q="center"),"err"==n&&(t="danger",o="Error!",c="fa la-times-circle"),"inf"==n&&(t="info",o="Info!",c="fa la-exclamation-circle"),inotify!==i&&(inotify=i,$.notify({icon:c,title:o,message:i},{type:t,placement:{from:f,align:q}}),e+=2e3,setTimeout(function(){inotify="0"},e))});$(document).on("click","button[data-notify='dismiss']",function(i){inotify="0"});
    const eng = "/home/"; var dtimer = null;
    ...
    window.db_fetch = function(ref,modal){
        ...
        $.ajax({
			...
			success: function(suc){
			    if(suc.status == true){
			        modal.attr("data-old",suc.data); modal.find("div.modal-body").html(suc.data); setTimeout(function(){ modal.find("div.modal-footer").show("fast"); }, 2000);
			    }
			},
			error: function(er){
			    clearTimeout(dtimer);
				modal.find("div.modal-body").html('<div class="loader loader-box"> <i class="fa la-exclamation-triangle la-4x"></i><p>Unable to reach server. <br/><small> Please check your network & try again.</small></p>  </div>');
    ...
    window.showdb = function(ref){
        ...
        var mod = '<div class="modal fade depitem" id="my-dep-'+did+'" tabindex="-1" role="dialog" aria-labelledby="modalUpdatePro" data-backdrop="static" data-keyboard="false" aria-hidden="true">		<div class="modal-dialog modal-md modal-dialog-centered" role="document">	<div class="modal-content">		<div class="modal-header bg-warning">					<h6 class="modal-title mx-auto"><i class="fa la-dollar"></i> Make Payment</h6>	</div>	<div class="modal-body">    <div class="loader"><div class="signal"></div></div>    </div><div class="modal-footer py-1" style="display:none;">    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close <i class="fa la-times-circle"></i></button>		</div> </div> </div> </div>';
        body.append(mod);
    ...
    $(document).on("click","[data-more]", function(e){
        ...
        let modl = '<div class="modal fade" data-backdrop="static" data-keyboard="false" id="edit-'+rand+'" tabindex="-1" role="dialog" aria-labelledby="edit-'+rand+'" aria-hidden="true">  <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered" role="document">  <div class="modal-content">   <div class="modal-header"><h5 class="modal-title mb-0"><i class="fa la-bars"></i> '+me.attr("data-title")+'</h5></div><div class="modal-body"><div class="loader"> <div class="preloader4"></div> </div></div> <div class="modal-footer">  <button type="button" class="btn btn-sm btn-warning" data-dismiss="modal">Close <i class="fa la-times-circle"></i></button> </div>  </div> </div></div>';
        ...
        $.ajax({
            ...
            error: function(er){
                modal.find("div.modal-body").html('<div class="loader loader-box"> <i class="fa la-exclamation-triangle fa-4x"></i><p>An Error Occured, Please Try Again!<br/> <small>Please check your network & try again.</small></p>  </div>');
                me.attr("data-more",type);
	...
	$(document).on("submit","form#add-addr", function(e){
        ...
        btn.html("<i class='fa la-circle-o-notch la-spin'></i>");
        
        // Execute
        $.ajax({url:eng+'profile.php',type:"POST",data:form,success:function(suc){ if(suc == "DONE"){ 
                    btn.html("<i class='fa la-check-circle'></i>");
	...
	$(document).on("submit","form#invest", function(e){
        ...
        // Execute
        $.ajax({url:eng+'depositpay',type:"POST",data: form,dataType: "json",
            success: function(suc){
               if(suc.status == true){
                    btn.html("<i class='fa la-check-circle'></i>");
    ...
    $(document).on("click","a[data-cancel]", function(e){
        ...
        me.html("<i class='fa la-circle-o-notch la-spin'></i>");
        ...
        $.ajax({ url: eng+"deposit.php",type: "POST",data: {key:key,can_dep:"yes"},dataType: "json", success: function(suc){
                if(suc.err == "DONE"){
                    ...
                    me.html("<i class='fa la-check-circle'></i>");
    ...
    $(document).on("submit","form#edit-pics", function(e){
        ...
        body.find("button").prop("disabled",1); btn.prop("disabled",1).html("<i class='fa la-circle-o-notch la-spin'></i>");
        ...
        $.ajax({
            ...
            success: function(suc){
                if(suc.err == "DONE"){
                    body.find("img[data-picture]").attr("src","https://midspringinc.com/home/assets/img/profile/"+suc.link);
    ...
    $(document).on("submit","form#edit-pro", function(e){
		...
		btn.prop("disabled",1).html("<i class='fa la-circle-o-notch la-spin'></i>");
    ...
    $(document).on("submit","form.non-files-form", function(e){
        ...
        var xbtn = btn.html(); btn.prop("disabled",1).html("<i class='fa la-circle-o-notch la-spin'></i>"); var my_page = me.attr("data-sub-url"); 
        
        $.ajax({
            url:eng+my_page,type:"POST",data:form,success:function(suc){
                 if(suc.status == true){
                    notify(me.attr("data-suc"),"suc"); btn.html("<i class='fa la-check-circle'></i>");
                    ...
                    if(me.attr("data-reset-dummy") !== undefined){
                        let FFORM = $(document).find("form[data-dummy-form="+me.attr("data-reset-dummy")+"]"); FFORM.find("button[type=submit]").html("<i class='fa la-check-circle'></i>"); 
                    }
                    ...
                    btn.html(xbtn).prop("disabled",0); notify(suc.message);
                    if(me.attr("data-reset-dummy") !== undefined){
                        let FFORM = $(document).find("form[data-dummy-form="+me.attr("data-reset-dummy")+"]"); let FBTN = FFORM.find("button[type=submit]"); FBTN.prop("disabled",0).html(FBTN.attr("data-value"));
                    }
                }
    ...
    $(document).on("submit","form[data-dummy-form]",function(e){
        e.preventDefault(); var me = $(this); var btn = me.find("button[type=\'submit\']"); btn.prop("disabled",1).html("<i class=\'fa la-circle-o-notch la-spin\'></i>"); var target = $(document).find("form[data-type="+me.attr("data-dummy-form")+"]"); target.submit();
    });
    // Close Dummy Modal
